...
{
    public partial class ThongKeBienBanViPham : Form
    {
        public ThongKeBienBanViPham()
        {
            InitializeComponent();
        }
        thongke_bienbanvipham a = new thongke_bienbanvipham();

        private void ThongKeBienBanViPham_Load(object sender, EventArgs e)
        {
        ...
        private void btt_xembaocao_Click(object sender, EventArgs e)
        {
            ...
            DateTime dt1 = Convert.ToDateTime(dateTimePicker1.Value.ToString());
            DateTime dt2 = Convert.ToDateTime(dateTimePicker2.Value.ToString());
            SqlCommand cmd = new SqlCommand("Select * from dbo.THONGKEBIENBANVIPHAM() WHERE NGAYLAP >= '" + dt1.ToString("yyyy-MM-dd") + "' and NGAYLAP <= '" + dt2.ToString("yyyy-MM-dd") + "'", con);
